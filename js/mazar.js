...
function app(){
  ...
  document.addEventListener('click', e => { const t = e.target.closest('[data-mazar],[data-ozer]'); if (!t) return; if (t.href && (e.metaKey || e.ctrlKey || e.shiftKey || e.altKey)) return;   /* a modified click on a link still opens it in a new tab */ e.preventDefault(); const closer = $('.drawer.is-open .drawer__close, .menu.is-open .menu__close, .is-menu-open .menu__close'); if (closer) closer.click(); if (FLOAT){ if (innerWidth > 900) setExpanded(true); open(true); } const ref = t.dataset.ref; if (ref && parseRef(ref)){ openPassage(ref); return; }   /* data-ref="John 15:4-5" opens that passage in the Bible tab */ const tab = t.dataset.mazar || t.dataset.ozer; if (tab) setTab(tab, true); else if (!FLOAT){ input.focus(); w.scrollIntoView({ behavior: 'smooth', block: 'start' }); } });
