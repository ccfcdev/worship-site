...
async function dashboardPage(modal){
  ...
  async function regsTab(){
    ...
    const rows = () => all.filter(x => (!ed || x.edition === ed) && (!s.value || (x.first_name + ' ' + (x.middle_name||'') + ' ' + x.surname + ' ' + x.phone + ' ' + (x.residence||'') + ' ' + (x.country||'') + ' ' + (x.email||'')).toLowerCase().includes(s.value.toLowerCase())));
